...
public class StringUT {
	...
	static {
		LEADING = Pattern.compile("^\\s+");
		TRAILING = Pattern.compile("\\s+$");
	}

	public static String getLeadingSpaces(final String str) {
		final Matcher m = StringUT.LEADING.matcher(str);
		if (m.find()) {
			return m.group(0);
		}
		return "";
	...
	public static String getTrailingSpaces(final String str) {
		final Matcher m = StringUT.TRAILING.matcher(str);
		if (m.find()) {
			return m.group(0);
		}
		return "";
	...
	public static String remove(final String msg, final String value) {
		return msg.replace(value, "");
	}

	public static String t(final String colorize) {
		return ChatColor.translateAlternateColorCodes('&', colorize);
	}

	public static String u(final String uncolorize) {
		return uncolorize.replace('§', '&');
	}
	...
	public static List<String> t(final List<String> colorize) {
		for (int x = 0; x < colorize.size(); ++x) {
			colorize.set(x, t(colorize.get(x)));
		}
		return colorize;
	}

	public static List<String> u(final List<String> uncolorize) {
		for (int x = 0; x < uncolorize.size(); ++x) {
			uncolorize.set(x, u(uncolorize.get(x)));
		}
		return uncolorize;
	}

	public static List<String> d(final List<String> remove) {
		for (int x = 0; x < remove.size(); ++x) {
			remove.set(x, d(remove.get(x)));
		}
		return remove;
	}
